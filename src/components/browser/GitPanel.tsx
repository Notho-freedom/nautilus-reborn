    ...
    >
      <div className="p-3 space-y-4">
        <div className="flex items-center gap-2">
          <GitBranch size={14} strokeWidth={1.5} className="text-primary" />
          ...
          {loading && <Loader2 size={12} strokeWidth={1.5} className="animate-spin text-muted-foreground" />}
        </div>

        {snapshot.error && (
          <div className="text-[10px] font-body text-error bg-error/10 rounded-md p-2">{snapshot.error}</div>
        )}
        ...
        <div className="pt-3">
          <h4 className="text-[10px] font-display text-muted-foreground/70 uppercase tracking-[0.18em] mb-2">Recent Commits</h4>
          <div className="space-y-1.5">
            {snapshot.recentCommits.map(commit => (
              <div key={commit.hash} className="flex items-start gap-1.5 text-xs font-body">
                <GitCommit size={11} strokeWidth={1.5} className="text-muted-foreground mt-0.5 shrink-0" />
        ...
        <div className="pt-3 space-y-2">
          <input
            ...
            className="w-full h-8 rounded-lg bg-notilus-surface-2/70 px-2 text-xs font-body text-foreground placeholder:text-muted-foreground outline-none focus:ring-1 focus:ring-primary/25"
          />
          <button
            onClick={() => void handleCommit()}
            className="w-full h-8 rounded-lg bg-primary/10 text-primary text-xs font-display tracking-wider hover:bg-primary/20 transition-colors disabled:opacity-50"
...
function FileSection({
  ...
  return (
    <div className="pt-3 space-y-1.5">
      <h4 className="text-[10px] font-display text-muted-foreground/70 uppercase tracking-[0.18em]">{title} ({files.length})</h4>
      {files.map(file => (
        <div key={file.path} className="flex items-center gap-1.5 text-xs font-body group rounded px-1 py-0.5 hover:bg-notilus-surface-2/40 transition-colors">
          <FileText size={11} strokeWidth={1.5} className={cn(color, 'shrink-0')} />
          ...
          <button onClick={() => onAction(file.path)} className="h-5 px-1.5 text-[9px] rounded-md bg-notilus-surface-2/60 text-muted-foreground hover:text-foreground hover:bg-notilus-surface-2 transition-colors opacity-0 group-hover:opacity-100" title={actionLabel}>
            <span className="inline-flex items-center gap-1"><ActionIcon size={9} strokeWidth={1.5} />{actionLabel}</span>
          </button>
