  ...
  return (
    <div className="p-3 space-y-4 overflow-y-auto scrollbar-thin flex-1">
      ...
      <div className="flex items-center gap-2">
        ...
        {snapshot.behind > 0 && (
          <span className="text-[10px] font-display bg-warning/15 text-warning px-2 py-0.5 rounded-md tracking-wider">
            ↓{snapshot.behind}
          </span>
        )}
      </div>
      ...
      <FileSection
        ...
        actionLabel="Unstage"
        actionIcon={Square}
        onAction={path => void unstageGitFile(path)}
      />
      <FileSection
        title="Changes"
        ...
        onAction={path => void stageGitFile(path)}
        destructiveActionLabel="Discard"
        onDestructiveAction={path => void discardGitFile(path)}
      />

      <div className="border-t border-border pt-3">
        <h4 className="text-[10px] font-display text-muted-foreground uppercase tracking-widest mb-2">
          Recent Commits
        </h4>
...
function FileSection({
  ...
  onDestructiveAction?: (path: string) => void;
}) {
  return (
      ...
      {files.map(file => (
        <div key={file.path} className="flex items-center gap-1.5 text-xs font-body group">
          ...
          <button
            ...
            className="h-5 px-1.5 text-[9px] rounded-md bg-notilus-surface-1 text-muted-foreground hover:text-foreground transition-colors opacity-0 group-hover:opacity-100"
            title={actionLabel}
          >
          ...
          {destructiveActionLabel && onDestructiveAction && (
            <button
              onClick={() => onDestructiveAction(file.path)}
              className="h-5 w-5 rounded-md text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors opacity-0 group-hover:opacity-100 flex items-center justify-center"
