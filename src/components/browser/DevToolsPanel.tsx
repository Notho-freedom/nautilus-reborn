...
export function DevToolsPanel({
  ...
  return (
    <div className="shrink-0 border-t border-border/20 bg-card/95 backdrop-blur-sm flex flex-col" style={{ height }}>
      <div
        onMouseDown={handleMouseDown}
        className={cn("h-1 cursor-row-resize hover:bg-primary/30 transition-colors duration-fast", isDragging && "bg-primary/50")}
      />

      <div className="flex items-center h-8 bg-card/45 px-1 gap-0.5 shrink-0">
        {TABS.map(tab => (
          <button
            key={tab.id}
