...
export function GitHubReposPanel({ onNavigate }: GitHubReposPanelProps) {
  ...
  const handleLoadRepos = async () => {
    ...
    setError(null);
    try {
      const loaded = await fetchGitHubRepos({
    ...
    <div className="p-3 space-y-3 overflow-y-auto scrollbar-thin flex-1">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="w-7 h-7 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-notilus-surface-1 transition-colors duration-fast"
      ...
      <form onSubmit={handleSaveConnection} className="space-y-1.5">
        <input
          ...
          placeholder="GitHub username (public repos)"
        ...
        <div className="grid grid-cols-[1fr_auto] gap-1.5">
          <input
            ...
            className="h-8 rounded-lg bg-notilus-surface-1 border border-border px-2 text-xs font-body text-foreground placeholder:text-muted-foreground outline-none"
          />
          <button
            type="submit"
            className="h-8 px-2 rounded-md notilus-gradient text-[10px] font-body text-primary-foreground"
          >
            Connect
          </button>
        </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className={`flex items-center gap-1 px-2 h-6 rounded-md text-[10px] font-body transition-colors duration-fast ${
            showPrivate ? 'bg-primary/15 text-primary' : 'bg-notilus-surface-1 text-muted-foreground'
          }`}
          title="Show private repos requires token"
        >
          <Lock size={9} /> Private
        </button>
        <button
          onClick={() =>
            setSortBy(current => (current === 'updated' ? 'stars' : current === 'stars' ? 'name' : 'updated'))
          }
      ...
      {error && (
        <div className="text-[10px] font-body text-error bg-error/10 border border-error/30 rounded-md p-2">
          {error}
        </div>
      )}
      ...
      <div className="space-y-2">
        {filtered.map(repo => (
          <button
            ...
            className="w-full text-left p-2.5 rounded-lg bg-notilus-surface-1 border border-border space-y-1.5 hover:bg-notilus-surface-2 transition-colors duration-fast"
          >
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="p-4 text-center text-xs font-body text-muted-foreground">
          {repos.length === 0 ? 'Connect GitHub to load repositories.' : 'No repositories found.'}
        </div>
      )}
    </div>
  );
