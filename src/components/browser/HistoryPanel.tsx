      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {Object.entries(grouped).map(([date, entries]) => (
          <div key={date}>
            ...
            {entries.map(h => (
              <div key={h.id} className="flex items-center gap-2 px-3 py-2 hover:bg-muted/50 transition-colors duration-fast cursor-pointer">
                <img
                ...
                <span className="text-[10px] font-body text-muted-foreground shrink-0">{h.time}</span>
              </div>
            ))}
          </div>
        ))}
