    ...
    <div className="flex flex-col h-full">
      <div className="p-3 border-b border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="w-6 h-6 flex items-center justify-center rounded bg-primary/15 text-primary hover:bg-primary/25 transition-colors">
            <Plus size={12} />
          </button>
        </div>
      ...
      <div className="flex gap-1 p-2 border-b border-border overflow-x-auto scrollbar-thin">
        <button
          ...
          className={`px-2 py-0.5 rounded text-[10px] font-mono shrink-0 transition-colors ${!activeFolder ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground'}`}
        >
          All
        </button>
        {folders.map(f => (
          <button
            ...
            className={`px-2 py-0.5 rounded text-[10px] font-mono shrink-0 transition-colors flex items-center gap-1 ${activeFolder === f ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground'}`}
          >
            <Folder size={9} /> {f}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {filtered.map(b => (
          <div key={b.id} className="flex items-center gap-2 px-3 py-2 hover:bg-muted/50 transition-colors cursor-pointer group">
            <img
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[11px] text-foreground truncate">{b.title}</div>
              <div className="text-[9px] text-muted-foreground truncate">{b.url}</div>
            </div>
            <div className="flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
              {b.tags.slice(0, 2).map(t => (
                <span key={t} className="flex items-center gap-0.5 px-1 py-0.5 rounded bg-secondary text-[8px] text-muted-foreground">
                  <Tag size={7} /> {t}
                </span>
              ))}
            </div>
